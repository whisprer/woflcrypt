...
# Convert hexadecimal character count, key, and direction to plaintext
...
print(f"Rotation direction (hexadecimal): {sht_dir}")
print(f"Rotation amount (plaintext): {rot_val}")
print(f"Rotation direction (hexadecimal): {hex_dir}")
print(f"Rotation amount (hexadecimal): {hex_rot}")
print(f"Encrypted RDN (hexadecimal): {hex_rdn}")
print(f"OG char count (plaintext): {txt_cnt}")
print(f"OG char count (hexadecimal): {hex_cnt}")
...
rotated_val = rot_hex(hex_msg, rot_val, direction='left' if sht_dir == 'right' else 'left')
