...
print(f"Direction count (hexadecimal): {hex_rdr}")
...
# Count chars in rdn i.e. integerstring, plaintext
rdr_cnt = cnt_rdr(rdn)

# convert rdn char count to hex
hex_rdr = txt_to_hex(str(rdr_cnt))  # Convert direction count to string before converting to hex
...
print(f"RDN count (integer): {rdr_cnt}, RDN count (hexadecimal): {hex_rdr}")